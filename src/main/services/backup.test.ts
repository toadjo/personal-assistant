...
describe("backup service", () => {
  ...
  it("exports and imports round-trip", () => {
    ...
    testDb.prepare(
      "INSERT INTO reminders (id, text, dueAt, recurrence, status, notifyChannel) VALUES (?, ?, ?, ?, ?, ?)"
    ...
    ).run("t1", "Task", "", null, "normal", "open", "none", "desktop", "2026-01-01T00:00:00Z", "2026-01-01T00:00:00Z", null);
    testDb.prepare(
  ...
  it("resetAllData clears all user tables", () => {
    ...
    testDb.prepare(
      "INSERT INTO renderer_errors (id, createdAt, message, stack, componentStack) VALUES (?, ?, ?, ?, ?)"
