...
export function useThemePreference() {
  ...
  const setCustomOverride = useCallback(
    (key: ThemeTokenKey, value: string | undefined) => {
      setState((prev) => {
        ...
        const next: ThemeState = {
          preset: prev.preset,
          custom: Object.keys(nextOverrides).length > 0
            ? makeCustomTheme(basePreset, nextOverrides)
            : undefined
        };
        writeThemeState(next);
        return next;
